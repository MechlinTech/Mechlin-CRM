...
const projectSchema = z.object({
  ...
  expected_end_date: z.string().optional().nullable().or(z.literal("")),
  ...
  // Only validate if BOTH dates are present and NOT empty strings
  if (!data.expected_end_date || !data.start_date || data.expected_end_date === "") return true;
...
export function CreateProjectForm({ onSuccess, project, organisations }: any) {
  ...
  async function onSubmit(values: z.infer<typeof projectSchema>) {
    ...
    // Clean data: convert empty strings for end_date to null so the server doesn't crash
    const submissionData = {
      ...values,
      expected_end_date: values.expected_end_date === "" ? null : values.expected_end_date
    };

    const result = isEditMode
      ? await updateProjectAction(project.id, submissionData)
      : await createProjectAction(submissionData);
