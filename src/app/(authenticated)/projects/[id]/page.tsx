...
export default async function ProjectOverview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: project, error } = await supabase
    .from("projects")
    .select("*, organisations(*), phases(*, milestones(*))")
    .eq("id", id)
    .single();
    ...
    <div className="max-w-5xl space-y-12 pb-20 text-black">
      ...
      <section className="p-10 border border-zinc-200 rounded-[32px] bg-zinc-50/30 shadow-sm">
        <div className="flex justify-between items-start mb-10">
          ...
          <Dialog>
            ...
            <DialogContent className="max-w-2xl bg-white text-black border-none shadow-2xl">
              <DialogHeader><DialogTitle className="font-bold">Edit Project Profile</DialogTitle></DialogHeader>
        ...
        <div className="grid grid-cols-4 gap-12 border-t border-zinc-100 pt-8">
          ...
          <a href={project.repo_link} target="_blank" className="text-blue-600 font-bold truncate block hover:underline text-sm">{project.repo_link || 'Link Repository'}</a></div>
        </div>
      </section>

      {/* 2. COLLAPSIBLE ROADMAP */}
        ...
        <div className="flex justify-between items-center px-4">
          ...
          <Dialog>
            ...
            <DialogContent className="bg-white text-black border-none shadow-2xl">
              <DialogHeader><DialogTitle className="font-bold">Initialize New Phase</DialogTitle></DialogHeader>
          ...
          {project.phases?.map((phase: any) => (
            <Collapsible key={phase.id} className="group border border-zinc-200 rounded-[24px] bg-white shadow-sm overflow-hidden transition-all hover:border-zinc-400">
              <div className="flex items-center justify-between p-3">
                <CollapsibleTrigger className="flex items-center gap-5 flex-1 p-5 hover:bg-zinc-50 transition-colors rounded-xl">
                ...
                <div className="flex gap-2 pr-6 items-center">
                  <Dialog>
                    <DialogTrigger asChild>
                       <button className="p-3 hover:bg-zinc-100 rounded-full text-zinc-400 hover:text-black transition-colors"><Pencil className="h-4 w-4" /></button>
                    </DialogTrigger>
                    <DialogContent className="bg-white text-black border-none"><DialogHeader><DialogTitle className="font-bold">Modify Phase</DialogTitle></DialogHeader><PhaseForm projectId={id} phase={phase} /></DialogContent>
                  </Dialog>
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button variant="ghost" size="sm" className="text-[10px] font-black uppercase text-zinc-500 hover:text-black hover:bg-zinc-50 h-9 px-4">+ Milestone</Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-xl bg-white text-black border-none">
                      <DialogHeader><DialogTitle className="font-bold">Configure Milestone</DialogTitle></DialogHeader>
