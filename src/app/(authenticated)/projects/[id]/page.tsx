...
export default function ProjectOverview({ params }: { params: any }) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20 px-4 sm:px-6 lg:px-0 text-[#0F172A]">
      {/* 1. TOP SECTION */}
              ...
              <div className="  flex flex-col md:flex-row items-center justify-between p-1"> 
                ...
                <div className="flex items-center justify-end gap-3 pr-2 w-full md:w-auto p-2 md:p-0">
                  <div className="flex items-center gap-1.5 p-1 bg-white rounded-md border border-slate-100 shadow-sm">
                    ...
                    <ActionButton title="Edit" trigger={<button className="h-8 w-8 flex items-center  justify-center rounded-md text-slate-500 hover:text-[#006AFF] transition-all"><Pencil className="h-4 w-4 cursor-pointer" /></button>}><PhaseForm projectId={id} phase={phase} onSuccess={loadData} /></ActionButton>
                  ...
                  <Dialog open={activeMilestonePhase === phase.id} onOpenChange={(open) => setActiveMilestonePhase(open ? phase.id : null)}>
                    <DialogTrigger asChild>
                      {/* FIXED: Milestone Button is now Primary Variant with White text */}
