...
export default function ProjectLayout({ children, params }: { children: React.ReactNode, params: any }) {
  ...
  return (
    // Added overflow-x-hidden to prevent the horizontal scrollbar during transitions
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F7F8FA] text-[#0F172A] font-sans relative overflow-x-hidden">
      {!isSidebarOpen && (
      ...
      {/* Updated flex-1 logic: 
          1. Added w-full to ensure it respects parent width.
          2. Added overflow-x-hidden to children container.
      */}
      <div className={cn(
        "flex-1 p-4 md:p-8 overflow-y-auto custom-scrollbar border-r border-slate-100 transition-all duration-300 w-full",
        !isSidebarOpen && "md:pr-8" // Maintain padding when sidebar is closed
      )}>
      ...
      {/* Updated Sidebar logic:
          1. isSidebarOpen ? "w-[280px]" : "w-0 overflow-hidden" 
          2. Removed sticky inside the collapsible logic to prevent it from holding space.
      */}
      <aside 
        data-app-sidebar 
        className={cn(
          "bg-white h-screen sticky top-0 flex flex-col transition-all duration-300 ease-in-out border-l border-slate-100", 
          isSidebarOpen ? "w-full md:w-[280px] opacity-100" : "w-0 opacity-0 overflow-hidden border-none"
        )}
      >
        <section className="shrink-0 p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="p-1.5 bg-white text-slate-500 rounded-lg hover:text-[#006AFF] border border-slate-200 cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
          <Collapsible defaultOpen>
            <CollapsibleTrigger className="flex items-center justify-between w-full p-2 bg-slate-50 rounded-xl group">
              <span className="text-sm font-medium uppercase text-slate-400 flex items-center gap-2">
                <ShieldCheck className="h-3 w-3 text-[#006AFF]" /> Mechlin Team
              </span>
            ...
            <CollapsibleContent className="space-y-1.5 mt-2 px-1">
              {mechlinTeam.map((m: any) => (
                <div key={m.users?.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg transition-colors group">
                  <div className="h-7 w-7 rounded-lg bg-[#006AFF]/10 text-[#006AFF] flex items-center justify-center text-[10px] font-semibold uppercase shrink-0 transition-colors group-hover:bg-[#006AFF] group-hover:text-white">
          ...
          <Collapsible defaultOpen>
            <CollapsibleTrigger className="flex items-center justify-between w-full p-2 bg-slate-50 rounded-xl group">
              <span className="text-sm font-medium uppercase text-slate-400 flex items-center gap-2">
                <Users className="h-3 w-3 text-[#006AFF]" /> Client Team
              </span>
              <ChevronDown className="h-4 w-4 text-slate-300 group-data-[state=open]:rotate-180 transition-transform" />
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-1.5 mt-2 px-1">
        ...
        <section className="p-5 border-t border-slate-100 bg-white flex flex-col h-[45%] shrink-0">
          <div className="flex justify-between items-center mb-5 shrink-0">
            <h3 className="text-sm font-medium uppercase text-slate-400 tracking-widest flex items-center gap-2">
              <Megaphone className="h-3 w-3 text-[#006AFF]" /> Broadcasts
            </h3>
            {!loading && hasPermission('pmupdates.create') && (
              <PMUpdateDialog projectId={id} onSuccess={loadData}>
                <button className="h-7 w-7 bg-[#006AFF] text-white rounded-lg flex items-center justify-center hover:bg-[#99C4FF] transition-all shadow-md active:scale-90 cursor-pointer">
