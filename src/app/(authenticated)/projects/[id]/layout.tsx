...
import { toast } from "sonner"; // Added for delete feedback
...
export default function ProjectLayout({ children, params }: { children: React.ReactNode, params: any }) {
  ...
  // UPDATE: Lifted loadData out of useEffect so it can be passed to Dialogs
      ...
      <aside className={cn("bg-white h-screen sticky top-0 flex flex-col border-l border-slate-200 shadow-sm transition-all duration-300 overflow-hidden shrink-0", isSidebarOpen ? "w-[340px]" : "w-0 border-none")}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <Collapsible defaultOpen>
            <CollapsibleTrigger className="flex items-center justify-between w-full p-2 bg-slate-50 rounded-xl group">
              <span className="text-sm font-black uppercase text-slate-400 flex items-center gap-2"><ShieldCheck className="h-3 w-3 text-[#4F46E5]" /> Mechlin Team</span>
        ...
        <section className="p-5 border-t border-slate-100 bg-white flex flex-col h-[45%] shrink-0">
          <div className="flex justify-between items-center mb-5 shrink-0">
            ...
            {/* UPDATE: Added onSuccess prop to trigger refresh after creation */}
            ...
            {pmNotices.map((log: any) => (
              <div key={log.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3 relative group hover:border-[#4F46E5]/50 transition-all">
                <div className="flex gap-1.5 absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white border border-slate-100 rounded-lg p-1 shadow-sm">
                  {/* UPDATE: Added onSuccess prop to trigger refresh after edit */}
                  <PMUpdateDialog projectId={id} log={log} onSuccess={loadData}>
                    <button className="p-1.5 text-slate-400 hover:text-[#4F46E5] cursor-pointer"><Pencil className="h-3 w-3" /></button>
                  </PMUpdateDialog>
                  {/* UPDATE: Added await and loadData() call for immediate removal */}
                  <form action={async () => { 
  ...
  <Dialog>
    <DialogTrigger asChild>
      <button className="text-[10px] font-black uppercase text-[#0F172A] hover:text-[#4F46E5] flex items-center gap-1 cursor-pointer transition-colors">
        <Eye className="h-3 w-3" /> View Full
      </button>
    </DialogTrigger>
    
    <DialogContent className="min-w-[100vh] bg-white text-slate-900 border-none shadow-2xl p-0 overflow-hidden rounded-[32px] flex flex-col min-h-[85vh]">
      
      {/* UPDATE: Replaced <div> with <DialogHeader> and <DialogTitle> for accessibility */}
      <DialogHeader className="p-6 bg-slate-50 border-b border-slate-100 shrink-0">
        <DialogTitle className="font-bold text-sm uppercase italic">
          Project Notice
        </DialogTitle>
        {/* Optional: Descriptive text for screen readers only */}
        <div className="sr-only">Detailed view of the project broadcast notice</div>
      </DialogHeader>

      <div className="p-8 overflow-y-auto prose prose-slate prose-sm flex-1" dangerouslySetInnerHTML={{ __html: log.new_value?.content }} />
    </DialogContent>
  </Dialog>
</div>
              </div>
