  ...
  return (
    <div className="max-w-5xl space-y-10 text-black font-sans relative">
        ...
        <div className="flex justify-between items-start border-b border-zinc-100 pb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
                ...
                <Badge className="bg-zinc-900 text-white border-none px-3 py-0.5 rounded-full text-[9px] font-bold uppercase shadow-sm">
                    {m.status}
                </Badge>
            </div>
            <p className="text-[10px] font-bold uppercase text-zinc-400 tracking-widest">Milestone Overview</p>
          </div>
          
          {/* Action Buttons moved inside card header */}
          <div className="flex gap-2">
            <ActionButton title="Upload Asset" trigger={<Button variant="outline" size="sm" className="rounded-full gap-2 border-zinc-300 font-bold uppercase text-[9px] h-9 px-4"><FileUp className="h-3.5 w-3.5" /> Upload</Button>}>
                <DocumentForm projectId={id} ids={{ phase_id: phaseId, milestone_id: milestoneId }} />
            </ActionButton>
            
            <Link href={`/projects/${id}/documents?milestoneId=${milestoneId}`} className="flex items-center h-9 px-4 bg-zinc-100 rounded-full font-bold uppercase text-[9px] gap-2 hover:bg-black hover:text-white transition-all shadow-sm">
                <FolderOpen className="h-3.5 w-3.5" /> Vault
            </Link>

            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
              <DialogTrigger asChild><Button variant="outline" size="sm" className="rounded-full gap-2 border-zinc-300 uppercase text-[9px] font-bold h-9 px-5 shadow-sm"><Pencil className="h-3 w-3" /> Edit</Button></DialogTrigger>
              <DialogContent className="bg-white text-black border-zinc-200 shadow-2xl"><DialogHeader><DialogTitle className="font-black uppercase tracking-widest text-lg">Edit Milestone</DialogTitle></DialogHeader>
                <MilestoneForm projectId={id} phaseId={phaseId} milestone={m} onSuccess={() => { setIsEditOpen(false); fetchData(); }} />
              </DialogContent>
            </Dialog>

            <Button variant="outline" size="sm" onClick={async () => { await deleteMilestoneAction(milestoneId, id); router.push(`/projects/${id}`); }} className="rounded-full border-red-100 text-red-600 hover:bg-red-50 uppercase text-[9px] font-bold gap-2 h-9 px-5 shadow-sm"><Trash2 className="h-3 w-3" /> Delete</Button>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {m.sprints?.map((sprint: any) => (
            <div key={sprint.id} className="p-6 bg-zinc-50/30 border border-zinc-100 rounded-[32px] flex items-center justify-between hover:border-black transition-all group shadow-sm">
                {/* FIXED: Path includes phaseId and milestoneId for correct navigation */}
      ...
      <section className="mt-8 border-t border-zinc-100 pt-10">
        <MilestoneThreads milestoneId={milestoneId} title="Milestone Discussions" />
      </section>
    </div>
  );
}